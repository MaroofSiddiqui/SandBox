...
public class JwtService {
	 ...
	 * JWT SECRET KEY
	 *
	 * @Value reads the value from application.properties or application.yml.
	 *
	 * Example:
	 *
	 * app.jwt.secret=BASE64_ENCODED_SECRET_KEY
	 *
	 * This secret is used to cryptographically sign JWTs and later verify that they
	 * were not modified.
	 */
	 ...
	 * GET SIGNING KEY
	 *
	 * Converts the configured Base64 secret string into a SecretKey that the JWT
	 * library can use for signing and verification.
	 */
	private SecretKey getSigningKey() {
		...
		/*
		 * Convert those bytes into an HMAC-compatible cryptographic signing key.
		 */
	 ...
	 *
	 * subject -> user's email userId -> user's database ID role -> SUPER_ADMIN / HR
	 * / CANDIDATE organizationId -> user's organization issuedAt -> token creation
	 * time expiration -> token expiry time
	 *
	...
	public String generateToken(User user) {
		 ...
		 *
		 * expiry = current time + configured expiration duration
		 */
		...
		return Jwts.builder()
				 ...
				 *
				 * We use the user's email because email is unique in our users table.
				 */
				 ...
				 *
				 * organization exists ↓ store organization ID
				 *
				 * organization is null ↓ store null
				 */
				.claim("organizationId", user.getOrganization() != null ? user.getOrganization().getId() : null)
				...
				/*
				 * Cryptographically sign the JWT using our secret key.
				 *
				 * This allows the backend to detect if someone modifies/tampers with the token.
				 */
	 ...
	 *
	 * This method is private because other methods such as extractEmail() and
	 * extractExpiration() use it internally.
	 */
	private Claims extractAllClaims(String token) {
		...
		return Jwts.parser()
				...
				/*
				 * Verify the JWT signature using the SAME secret key that was used to sign it.
				 *
				 * If someone changes the JWT contents, signature verification will fail.
				 */
				 ...
				 *
				 * Invalid, malformed, tampered, or expired tokens may cause JWT-related
				 * exceptions.
				 */
				 ...
				 *
				 * sub userId role organizationId iat exp
				 */
				.getPayload();
	}
	 ...
	 * Example:
	 *
	 * expiration = 10:00 PM current = 11:00 PM
	 *
	 * expiration.before(current) ↓ true
	 *
	 ...
	 *
	 * 1. Token email matches the database user's email 2. Token has not expired 3.
	...
	public boolean isTokenValid(String token, User user) {
		...
		// Expired JWT must not be accepted
		if (isTokenExpired(token)) {
			return false;
		}

		// Inactive user cannot use an existing JWT
		if (!"ACTIVE".equals(user.getStatus())) {
			return false;
		}

		// SUPER_ADMIN has no organization, so this check is skipped for them
